...
if __name__ == "__main__":
    chopsticksSemaphoreList = list()          #this list will hold one semaphore per chopstick
    numberOfPhilosophers = 5

    for i in range(numberOfPhilosophers):             
        chopsticksSemaphoreList.append(multiprocessing.Semaphore(1))    #one semaphore per chopstick

    seats = multiprocessing.Semaphore(numberOfPhilosophers - 1) # 1 semaphore to represent the seats at the table (numberOfPhilosophers - 1)

    philosopherProcessList = list()
    for i in range(numberOfPhilosophers): #instantiate all processes representing philosophers
        philosopherProcessList.append(multiprocessing.Process(target=philosopher, args=(i, chopsticksSemaphoreList, seats)))
