...
def philosopher(id: int, chopstick: list):
    ...
    for _ in range(6): #to make testing easier, instead of a forever loop we use a finite loop
        ...
        # Odd id philosophers take the left chopstick first.
        # Even id philosophers take the right chopstick first.
        if id % 2 == 0:
        ...
        chopstick[firstChopstick].acquire()
        print(f"DEBUG: philosopher{id} has chopstick{firstChopstick}")
        chopstick[secondChopstick].acquire()
        print(f"DEBUG: philosopher{id} has chopstick{secondChopstick}")

        eatForAWhile()  #use this line as is

        print(f"DEBUG: philosopher{id} is to release chopstick{secondChopstick}")
        chopstick[secondChopstick].release()
        print(f"DEBUG: philosopher{id} is to release chopstick{firstChopstick}")
        chopstick[firstChopstick].release()
